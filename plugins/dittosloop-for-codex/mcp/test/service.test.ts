...
test("renders workspace loop directory files from stored loop state", async () => {
  ...
  expect(verificationFile).not.toMatch(/status:\s*(passed|failed|needs_human|通过|失败|需要人工)/i);
...
test("recordValidatorResult finalizes v2 verification from a separate rubric agent", async () => {
  ...
  expect(detail).toMatchObject({
    run: { status: "completed", codexSession: { status: "started" } },
...
test("keeps a completed run's Codex session pending until a real thread is attached", async () => {
  const service = await createService();
  const loop = await createFormalLoop(service, {
    title: "AI Dev Tools Update Monitor",
    goal: "Generate the daily update report"
  });
  const launch = await service.startCodexSessionRun(loop.id, {
    goal: "Generate today's report"
  });

  const completed = await service.recordSessionResult(launch.run.id, {
    status: "passed",
    summary: "Report generated by the workflow runtime."
  });
  ...
  expect(completed.codexSession?.threadId).toBeUndefined();
  expect(completed.codexSession?.threadUrl).toBeUndefined();
...
test("normalizes persisted completed Codex sessions without a real thread back to pending", async () => {
  ...
  const statePath = join(store.dataDir, "state.json");
  const persisted = JSON.parse(await readFile(statePath, "utf8"));
  persisted.runs = persisted.runs.map((run: LoopRun) =>
    run.id === launch.run.id
      ? {
          ...run,
          status: "completed",
          completedAt: fixedTime,
  ...
  const snapshot = await service.getSnapshot();
  const snapshotRun = snapshot.runs.find((run) => run.id === launch.run.id);
  expect(snapshotRun?.codexSession).toMatchObject({
    status: "requested"
  });
