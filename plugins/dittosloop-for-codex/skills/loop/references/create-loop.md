...
1. 先复述推断出的 loop 目标、边界、触发方式、触发条件和预期输出。
2. 对低风险细节使用合理默认值，不要为每个缺失字段都追问。
3. 只对影响安全、权限、成本、破坏性操作、外部副作用、项目绑定、触发条件或验证的缺失信息追问。
...
`Loop Draft` 中的触发条件要单独成段，至少说明：

- 触发类型：手动触发、定时触发、事件触发、一次性运行，或只创建不运行。
- 如果是定时或重复触发：频率、时区、开始/停止条件、失败重试策略、是否每次需要人工确认。
- 如果是外部平台监控：登录态、验证码、公开访问限制、是否允许用户提供链接/截图作为输入。
- 当前会话主动触发并创建新 worker 时的执行归属：由新建的可见 Codex worker thread 执行；源会话只负责触发、绑定和查看状态。系统定时或事件触发直接打开的新会话本身就是 worker。

## 创建流程

1. 塑造 loop 合同：标题、目标或意图、触发方式和触发条件、脚本编排方式、runtime script 职责，以及已确认或可安全推断的 verification contract。
