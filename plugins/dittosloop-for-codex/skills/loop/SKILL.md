...
3. 用户明确同意后，先判断反馈应该落到哪里：当前 workflow、任务输出要求、verification rubrics、运行记录，还是长期记忆。
4. 如果反馈会影响当前 loop 之后的执行方式，应通过 workflow revision 更新当前 loop；如果反馈会影响验收标准，应更新 verification rubrics。
